    ...
    <div class="body-box" :style="{ gap: cardGap, padding: bodyPadding }">
      <div class="platform-card-item" v-for="platform in platforms" :key="platform.id" :style="cardItemStyle">
        <div class="platform-card-inner" :style="{ borderColor: platform.color, height: cardHeight }"
          ...
          <div class="platform-description">{{ platform.desc }}</div>
        </div>
      </div>
    </div>
    <!-- 轮播提示文本（绑定父组件传递的阈值） -->
          ...
          <div class="all-platforms-list">
            <div v-for="platform in allPlatforms" :key="platform.id" class="all-platform-item">
              ...
              <div class="item-info">
                ...
                <p class="item-desc">{{ platform.desc }}</p>
                <div class="item-meta">
...
export default {
  ...
  data() {
    return {
      allPlatforms: [
        { id: 1, name: 'AI中台', desc: '模型训练与资源管理', url: 'http://136.192.111.150:32088/', icon: '📊', status: 'active', color: '#1A5CD7' },
        { id: 2, name: '智能体平台', desc: '实时监控与智能告警系统', url: 'http://136.192.230.61:30050/apps', icon: '🔔', status: 'active', color: '#1A5CD7' },
        { id: 3, name: 'Dify平台', desc: '工作流搭建', url: 'http://136.192.230.65:8081/', icon: '🧠', status: 'active', color: '#1A5CD7' },
        { id: 4, name: 'RagFlow', desc: '知识库管理', url: 'http://136.192.230.68:8081/', icon: '⚙️', status: 'active', color: '#1A5CD7' },
        { id: 5, name: '数据集集市', desc: '海量数据收集与分析', url: 'http://136.192.111.150:30188/micro/modelZoo/maasDataset', icon: '📝', status: 'active', color: '#1A5CD7' },
        { id: 6, name: '模型训练', desc: '模型训练', url: 'http://136.192.111.150:30188/micro/modelZoo/maasModel/maasModelTune', icon: '🛡️', status: 'maintenance', color: '#1A5CD7' },
        { id: 7, name: '数据可视化平台', desc: '多维度数据图表展示', url: '/data-visual', icon: '📈', status: 'active', color: '#1A5CD7' },
        { id: 8, name: '资源管理', desc: '统一身份认证与权限管理', url: 'http://136.192.111.150:30188/micro/modelZoo/maasCluster', icon: '👥', status: 'active', color: '#1A5CD7' }
      ],
      platforms: [],
      carouselTimer: null,
      currentStartIndex: 0,
